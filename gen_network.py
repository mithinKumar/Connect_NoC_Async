...
def prepare_graph_file(gv_filename, layout="invalid", custom_topology_info=None):
    ...
    try:
        ...
        print("Could not open file " + gv_filename)
        sys.exit(-1)

    # Write header
    dot.write('digraph ' + options.topology + '_' +
              str(options.num_routers) + 'routers {\n')
    ...
    # For mesh and torus position nodes
        ...
        for c in range(options.routers_per_column):
            for r in range(options.routers_per_row):
                dot.write('\nnode [label="R'+str(rt_id)+'" pos="' +
                          str(r*1.5)+','+str(c*1.5)+'!"] R'+str(rt_id)+'\n')
                rt_id = rt_id+1
        dot.write('\n')
        ...
        if (options.graph_nodes):  # Add endpoint nodes
            ...
            for c in range(options.routers_per_column):
                for r in range(options.routers_per_row):
                    dot.write('\nnode [label="N'+str(rt_id)+'" pos="' +
                              str(r*1.5+0.75)+','+str(c*1.5+0.75)+'!"] N'+str(rt_id)+'\n')
                    rt_id = rt_id+1
    ...
    # For Fat Tree position nodes
        ...
        num_stages = int(math.log(options.num_routers, 2)) - \
            1  # counts router stages
        ...
        for stage in range(num_stages):
            if (stage < num_stages-1):  # not the top stage
                for sr in range(rts_in_stage):  # subrouter in particular stage
                    dot.write('\nnode [label="R'+str(rt_id)+'" pos="' +
                              str(sr*2)+','+str(stage*4)+'!"] R'+str(rt_id)+'\n')
                    rt_id = rt_id+1
                ...
                for sr in range(rts_in_stage/2):  # subrouter in particular stage
                    dot.write('\nnode [label="R'+str(rt_id)+'" pos="' +
                              str(1+sr*4)+','+str(stage*4)+'!"] R'+str(rt_id)+'\n')
                    rt_id = rt_id+1
        dot.write('\n')
        ...
        if (options.graph_nodes):  # Add endpoint nodes
            ...
            for sr in range(rts_in_stage):  # subrouter in particular stage
                dot.write('\nnode [label="N'+str(sr*2)+'" pos="' +
                          str(sr*2-0.5)+','+str(-2)+'!"] N'+str(sr*2)+'\n')
                dot.write('\nnode [label="N'+str(sr*2+1)+'" pos="' +
                          str(sr*2+0.5)+','+str(-2)+'!"] N'+str(sr*2+1)+'\n')
                # dot.write('node [label="N'+str(r)+'"] N'+str(r)+';\n');

    # For Butterfly position nodes
    elif (options.topology == "butterfly"):  # arrange nodes in left-to-right stages
        num_stages = int(math.log(options.num_routers, 2)
                         )  # counts router stages
        rts_in_stage = options.num_routers/2
        rt_id = 0
        for stage in range(num_stages):
            for sr in range(rts_in_stage):  # subrouter in particular stage
                ...
                dot.write('\nnode [label="R'+str(rt_id)+'" pos="' +
                          str(stage*4)+','+str(sr*2)+'!"] R'+str(rt_id)+'\n')
                rt_id = rt_id+1
        dot.write('\n')
        ...
        if (options.graph_nodes):  # Add endpoint nodes
            ...
            for sr in range(rts_in_stage):  # subrouter in particular stage
                ...
                dot.write('\nnode [label="N" pos="'+str(-2)+',' +
                          str(sr*2-0.5)+'!"] N'+str(sr*2+options.num_routers)+'\n')
                dot.write('\nnode [label="N" pos="'+str(-2)+','+str(sr *
                          2+0.5)+'!"] N'+str(sr*2+1+options.num_routers)+'\n')
                # right nodes
                ...
                dot.write('\nnode [label="N'+str(sr*2)+'" pos="' +
                          str(num_stages*4-2)+','+str(sr*2-0.5)+'!"] N'+str(sr*2)+'\n')
                dot.write('\nnode [label="N'+str(sr*2+1)+'" pos="' +
                          str(num_stages*4-2)+','+str(sr*2+0.5)+'!"] N'+str(sr*2+1)+'\n')
                # dot.write('node [label="N'+str(r)+'"] N'+str(r)+';\n');
    ...
    # For Unidirectional single switch topology only draw one router
        ...
        if (options.graph_nodes):  # Add endpoint nodes
            ...
            for r in range(options.send_endpoints):
                dot.write('node [label="N'+str(r+options.recv_endpoints) +
                          '"] N'+str(r+options.recv_endpoints)+';\n')
    ...
    # For Unidirectional Tree position nodes
        ...
        # counts link stages
        num_stages = int(
            math.ceil(math.log(num_leaf_nodes, options.uni_tree_fanout))) - 1
        ...
        cur_rt = 0
        rts_in_stage = 1
        # num_leaf_nodes/(1.0*options.uni_tree_fanout)
        last_stage_width = (1.0*options.uni_tree_fanout)**num_stages
        if (options.graph_nodes):  # Make more space to add endpoint nodes
            # num_leaf_nodes/(1.0*options.uni_tree_fanout)
            last_stage_width = (1.0*options.uni_tree_fanout)**(num_stages+1)
        ...
        for stage in range(num_stages+1):
            ...
            for r in range(rts_in_stage):
                dot.write('\nnode [label="R'+str(cur_rt)+'" pos="'+str(offset +
                          gap*r*1.0)+','+str((-1)*stage*2)+'!"] R'+str(cur_rt)+'\n')
                cur_rt += 1
                ...
                if (stage == 0):  # root
                    ...
                    if (build_down_tree):  # down tree has inputs at root
                        for i in range(options.uni_tree_inputs):
                            ...
                            dot.write('\nnode [label="N' + "" + '" pos="'+str(offset+gap*i*1.0)+','+str(
                                2)+'!"] N'+str(i+options.uni_tree_outputs)+'\n')
                ...
                if (stage == num_stages):  # last stage
                    ...
                    if (build_down_tree):  # down tree has outputs at leafs
                        if (options.uni_tree_distribute_leaves):  # distribute leaves
                            for o in range(options.uni_tree_outputs):
                                rt = get_uni_tree_distributed_rt(
                                    o, num_stages, options.uni_tree_fanout)
                                r_off = o / \
                                    (options.uni_tree_fanout**(num_stages))
                                dist = (rt*options.uni_tree_fanout+r_off)
                                # dot.write('\nnode [label="N'+ "" +'" pos="'+str(offset+jump*a*1.0+b*gap)+','+str(2)+'!"] N'+str(i+options.uni_tree_outputs)+'\n');
                                dot.write('\nnode [label="N'+str(o)+'" pos="'+str(
                                    offset+dist*1.0*gap)+','+str((-1)*(stage+1)*2)+'!"] N'+str(o)+'\n')
                        else:
                            for o in range(options.uni_tree_outputs):
                                dot.write('\nnode [label="N'+str(o)+'" pos="'+str(
                                    offset+gap*o*1.0)+','+str((-1)*(stage+1)*2)+'!"] N'+str(o)+'\n')
                                # dot.write('\nnode [label="R'+str(cur_rt)+'" pos="'+str(offset+gap*r*1.0)+','+str((-1)*stage*2)+'!"] R'+str(cur_rt)+'\n');
                    else:  # up tree has inputs at leafs
                        if (options.uni_tree_distribute_leaves):  # distribute leaves
                            for i in range(options.uni_tree_inputs):
                                rt = get_uni_tree_distributed_rt(
                                    i, num_stages, options.uni_tree_fanout)
                                r_off = i / \
                                    (options.uni_tree_fanout**(num_stages))
                                dist = (rt*options.uni_tree_fanout+r_off)
                                # dot.write('\nnode [label="N'+ "" +'" pos="'+str(offset+jump*a*1.0+b*gap)+','+str(2)+'!"] N'+str(i+options.uni_tree_outputs)+'\n');
                                dot.write('\nnode [label="N' + "" + '" pos="'+str(offset+dist*1.0*gap)+','+str(
                                    (-1)*(stage+1)*2)+'!"] N'+str(i+options.uni_tree_outputs)+'\n')
                        else:
                            for i in range(options.uni_tree_inputs):
                                dot.write('\nnode [label="N' + "" + '" pos="'+str(offset+gap*i*1.0)+','+str(
                                    (-1)*(stage+1)*2)+'!"] N'+str(i+options.uni_tree_outputs)+'\n')
...
def gen_single_switch_links(links, dot_filename, dump_topology_filename):
    ...
    if options.dump_topology_file:
        try:
            topo_dump = open(dump_topology_filename, 'w')
        except IOError:
            print("Could not open file " + dump_topology_filename)
            sys.exit(-1)

    # Generate graphviz .gv file
    if options.gen_graph:
        dot = prepare_graph_file(dot_filename + ".gv", "circo")

    # Expose user send/receive ports
    for r in range(options.num_routers):
        links.write(
            'send_ports_ifaces['+str(r)+'] = routers['+str(0)+'].in_ports['+str(r)+'];\n')
        links.write(
            'recv_ports_ifaces['+str(r)+'] = routers['+str(0)+'].out_ports['+str(r)+'];\n')
        if options.dump_topology_file:
        ...
        if options.gen_graph and options.graph_nodes:  # also include the endpoints
            dot.write('N'+str(r)+' -> R'+str(0) +
                      ' [ headlabel = "' + str(r) + '" ];\n')
            dot.write('R'+str(0)+' -> N'+str(r) +
                      ' [ taillabel = "' + str(r) + '" ];\n')
        # links.write('router_info_ifaces['+str(r)+'] =  get_router_info_ifc('+str(r)+');\n')
        links.write(
            'recv_ports_info_ifaces['+str(r)+'] =  get_port_info_ifc('+str(r)+');\n')
    ...
    if options.dump_topology_file:
        topo_dump.close()
    if options.verbose and options.dump_topology_file:
        print('Dumped topology file: ' + dump_topology_filename)
    # Close graphviz file
    if options.gen_graph:
        dot.write('}\n')
        dot.close()
    if options.verbose and options.gen_graph:
        print('Generated graphviz file: ' + dot_filename + ".gv")
...
def gen_single_switch_routing(file_prefix, dump_routing_filename):
    ...
    if options.dump_routing_file:
        try:
            ...
            print("Could not open file " + dump_routing_filename)
            sys.exit(-1)
    ...
    for dst in range(options.num_routers):
        ...
        rt.write('%x\n' % (out_port))
        if options.verbose:
            print('route:'+str(0)+'->'+str(dst)+':'+str(out_port))
        if options.dump_routing_file:
            route_dump.write('R'+str(0)+': '+str(dst) +
                             ' -> '+str(out_port)+'\n')

    rt.close()
    if options.verbose:
        print('Generated routing file: ' + filename)

    # Close routing dump file
    if options.dump_routing_file:
        route_dump.close()
    if options.verbose and options.dump_routing_file:
        print('Dumped routing file: ' + dump_routing_filename)
...
def gen_line_links(links, dot_filename, dump_topology_filename):
    ...
    if options.gen_graph:
        dot = prepare_graph_file(dot_filename + ".gv", "dot")

    # Expose user send/receive ports
    for r in range(options.num_routers):
        links.write('send_ports_ifaces['+str(r) +
                    '] = routers['+str(r)+'].in_ports[0];\n')
        links.write('recv_ports_ifaces['+str(r) +
                    '] = routers['+str(r)+'].out_ports[0];\n')
        if options.dump_topology_file:
            topo_dump.write('SendPort '+str(r)+' -> R'+str(r)+':'+str(0)+'\n')
            topo_dump.write('RecvPort '+str(r)+' -> R'+str(r)+':'+str(0)+'\n')
        if options.gen_graph and options.graph_nodes:  # also include the endpoints
            dot.write('N'+str(r)+' -> R'+str(r) +
                      ' [ headlabel = "' + '0' + '" ];\n')
            dot.write('R'+str(r)+' -> N'+str(r) +
                      ' [ taillabel = "' + '0' + '" ];\n')
        # links.write('router_info_ifaces['+str(r)+'] =  get_router_info_ifc('+str(r)+');\n')
        links.write(
            'recv_ports_info_ifaces['+str(r)+'] =  get_port_info_ifc('+str(r)+');\n')
    ...
    for r in range(options.num_routers-1):
        ...
        links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                    str(r)+'], 1, routers['+str(next_router)+'], 1);\n')
        if options.dump_topology_file:
            topo_dump.write('RouterLink '+'R'+str(r)+':'+str(1) +
                            ' -> '+'R'+str(next_router)+':'+str(1)+'\n')
        if options.gen_graph:
            dot.write('R'+str(r)+' -> R'+str(next_router) +
                      ' [ taillabel = "' + '1' + '", headlabel = "' + '1' + '" ];\n')
    ...
    for r in range(1, options.num_routers):
        ...
        links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                    str(r)+'], 2, routers['+str(prev_router)+'], 2);\n')
        if options.dump_topology_file:
            topo_dump.write('RouterLink '+'R'+str(r)+':'+str(2) +
                            ' -> '+'R'+str(prev_router)+':'+str(2)+'\n')
        if options.gen_graph:
            dot.write('R'+str(r)+' -> R'+str(prev_router) +
                      ' [ taillabel = "' + '2' + '", headlabel = "' + '2' + '" ];\n')
    ...
    for src in range(options.num_routers):
        ...
        for dst in range(options.num_routers):
            if src == dst:  # packet is destined to me, extract from router, i.e. out_port 0
                ...
                # str.format(  );
                if options.verbose:
                    print('route:'+str(src)+'->'+str(dst)+':'+'0')
                if options.dump_routing_file:
                    route_dump.write('R'+str(src)+': ' +
                                     str(dst)+' -> '+str(0)+'\n')
    ...
    for src in range(options.num_routers):
        ...
        for dst in range(options.num_routers):
            if src == dst:  # packet is destined to me, extract from router, i.e. out_port 0
                ...
                diff = dst-src
                dist = abs(diff)
                # Send clockwise, i.e. using out_port 1
                if (diff >= 0 and dist <= options.num_routers/2) or (diff < 0 and dist > options.num_routers/2):
                    out_port = 1
                    rt.write('%x\n' % (out_port))
                    if options.verbose:
                        print('route:'+str(src)+'->' +
...
def gen_star_links(links, dot_filename, dump_topology_filename):
    ...
    # Generate graphviz .gv file
    if options.gen_graph:
        # neato also works well for small networks
        dot = prepare_graph_file(dot_filename + ".gv", "circo")

    # Expose user send/receive ports
    for r in range(options.num_routers-1):
        links.write(
            'send_ports_ifaces['+str(r)+'] = routers['+str(r+1)+'].in_ports[0];\n')
        links.write(
            'recv_ports_ifaces['+str(r)+'] = routers['+str(r+1)+'].out_ports[0];\n')
        if options.dump_topology_file:
            topo_dump.write('SendPort '+str(r)+' -> R' +
                            str(r+1)+':'+str(0)+'\n')
            topo_dump.write('RecvPort '+str(r)+' -> R' +
                            str(r+1)+':'+str(0)+'\n')
        if options.gen_graph and options.graph_nodes:  # also include the endpoints
            dot.write('N'+str(r)+' -> R'+str(r+1) +
                      ' [ headlabel = "' + '0' + '" ];\n')
            dot.write('R'+str(r+1)+' -> N'+str(r) +
                      ' [ taillabel = "' + '0' + '" ];\n')
        # links.write('router_info_ifaces['+str(r)+'] =  get_router_info_ifc('+str(r)+');\n')
        links.write(
            'recv_ports_info_ifaces['+str(r)+'] = get_port_info_ifc('+str(r)+');\n')
    ...
    for r in range(1, options.num_routers):
        link_id = link_id + 1
        links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                    str(0)+'], ' + str(r-1) + ', routers['+str(r)+'], 1);\n')
        if options.dump_topology_file:
            topo_dump.write('RouterLink '+'R'+str(0)+':' +
                            str(r-1)+' -> '+'R'+str(r)+':'+str(1)+'\n')
        if options.gen_graph:
            dot.write('R'+str(0)+' -> R'+str(r) +
                      ' [ taillabel = "' + str(r-1) + '", headlabel = "' + '1' + '" ];\n')

    # Connect sattelite routers to central router (0).
    for r in range(1, options.num_routers):
        link_id = link_id + 1
        links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                    str(r)+'], ' + str(1) + ', routers['+str(0)+'], ' + str(r-1) + ');\n')
        if options.dump_topology_file:
            topo_dump.write('RouterLink '+'R'+str(r)+':' +
                            str(1)+' -> '+'R'+str(0)+':'+str(r-1)+'\n')
        if options.gen_graph:
            dot.write('R'+str(r)+' -> R'+str(0) +
                      ' [ taillabel = "' + str(1) + '", headlabel = "' + str(r-1) + '" ];\n')
    ...
    for src in range(options.num_routers):
        ...
        for dst in range(options.num_routers):
            if (src == 0):  # routing for central node
                ...
                rt.write('%x\n' % (out_port))
                if options.verbose:
                    print('route:'+str(src)+'->'+str(dst)+':'+str(out_port))
                if options.dump_routing_file:
                    route_dump.write('R'+str(src)+': ' +
                                     str(dst)+' -> '+str(out_port)+'\n')
...
def gen_mesh_links(links, dot_filename, dump_topology_filename):
    ...
    # Generate graphviz .gv file
    if options.gen_graph:
        dot = prepare_graph_file(dot_filename + ".gv", "neato")
    ...
    for r in range(1, options.routers_per_row):
        for c in range(0, options.routers_per_column):
            ...
            link_id = link_id + 1
            left_router = c*options.routers_per_row + \
                (r+options.routers_per_row-1) % options.routers_per_row
            links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                        str(rt_id)+'], 1, routers['+str(left_router)+'], 1);\n')
            if options.dump_topology_file:
                topo_dump.write('RouterLink '+'R'+str(rt_id)+':' +
                                str(1)+' -> '+'R'+str(left_router)+':'+str(1)+'\n')
            if options.gen_graph:
                dot.write('R'+str(rt_id)+' -> R'+str(left_router) +
                          ' [ taillabel = "' + '1' + '", headlabel = "' + '1' + '" ];\n')
    ...
    for r in range(0, options.routers_per_row):
        for c in range(1, options.routers_per_column):
            rt_id = c*options.routers_per_row + r
            link_id = link_id + 1
            up_router = ((c+options.routers_per_column-1) %
                         options.routers_per_column) * options.routers_per_row + r
            links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                        str(rt_id)+'], 2, routers['+str(up_router)+'], 2);\n')
            if options.dump_topology_file:
                topo_dump.write('RouterLink '+'R'+str(rt_id)+':' +
                                str(2)+' -> '+'R'+str(up_router)+':'+str(2)+'\n')
            if options.gen_graph:
                dot.write('R'+str(rt_id)+' -> R'+str(up_router) +
                          ' [ taillabel = "' + '2' + '", headlabel = "' + '2' + '" ];\n')
    ...
    for r in range(0, options.routers_per_row-1):
        for c in range(0, options.routers_per_column):
            rt_id = c*options.routers_per_row + r
            link_id = link_id + 1
            right_router = c*options.routers_per_row + \
                (r+options.routers_per_row+1) % options.routers_per_row
            links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                        str(rt_id)+'], 3, routers['+str(right_router)+'], 3);\n')
            if options.dump_topology_file:
                topo_dump.write('RouterLink '+'R'+str(rt_id)+':' +
                                str(3)+' -> '+'R'+str(right_router)+':'+str(3)+'\n')
            if options.gen_graph:
                dot.write('R'+str(rt_id)+' -> R'+str(right_router) +
                          ' [ taillabel = "' + '3' + '", headlabel = "' + '3' + '" ];\n')
    ...
    for r in range(0, options.routers_per_row):
        for c in range(0, options.routers_per_column-1):
            rt_id = c*options.routers_per_row + r
            link_id = link_id + 1
            down_router = ((c+options.routers_per_column+1) %
                           options.routers_per_column) * options.routers_per_row + r
            links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                        str(rt_id)+'], 4, routers['+str(down_router)+'], 4);\n')
            if options.dump_topology_file:
                topo_dump.write('RouterLink '+'R'+str(rt_id)+':' +
                                str(4)+' -> '+'R'+str(down_router)+':'+str(4)+'\n')
            if options.gen_graph:
                dot.write('R'+str(rt_id)+' -> R'+str(down_router) +
                          ' [ taillabel = "' + '4' + '", headlabel = "' + '4' + '" ];\n')

    # Expose the extra user ports
    if (options.expose_unused_ports):
        num_total_user_ports = options.num_routers + 2 * \
            options.routers_per_row + 2*options.routers_per_column
        for user_port in range(options.num_routers, num_total_user_ports):
            ...
            if (out_port == 4):
                c_offset = 1.25
            links.write(
                'send_ports_ifaces['+str(user_port)+'] = routers['+str(rt)+'].in_ports['+str(in_port)+'];\n')
            links.write(
                'recv_ports_ifaces['+str(user_port)+'] = routers['+str(rt)+'].out_ports['+str(out_port)+'];\n')

            if options.dump_topology_file:
                topo_dump.write('SendPort '+str(user_port) +
                                ' -> R'+str(rt)+':'+str(in_port)+'\n')
                topo_dump.write('RecvPort '+str(user_port) +
                                ' -> R'+str(rt)+':'+str(out_port)+'\n')
            if options.gen_graph and options.graph_nodes:  # also include the endpoints
                # dot.write('\nnode [label="N'+str(user_port)+'"] N'+str(user_port)+'\n');
                dot.write('\nnode [label="N'+str(user_port)+'" pos="'+str(
                    r*1.5+r_offset)+','+str(c*1.5+c_offset)+'!"] N'+str(user_port)+'\n')
                dot.write('N'+str(user_port)+' -> R'+str(rt) +
                          ' [ headlabel = "' + str(in_port) + '" ];\n')
                dot.write('R'+str(rt)+' -> N'+str(user_port) +
                          ' [ taillabel = "' + str(out_port) + '" ];\n')
            # links.write('router_info_ifaces['+str(user_port)+'] = get_router_info_ifc('+str(user_port)+');\n')
            links.write(
                'recv_ports_info_ifaces['+str(user_port)+'] =  get_port_info_ifc('+str(user_port)+');\n')
...
def gen_mesh_routing(file_prefix, dump_routing_filename):
    ...
    if (not options.expose_unused_ports):
        for src in range(options.num_routers):
            filename = options.output_dir + '/' + \
                file_prefix + str(src) + '.hex'
            try:
                rt = open(filename, 'w')
            except IOError:
                print("Could not open file " + filename)
                ...
                if src == dst:  # packet is destined to me, extract from router, i.e. out_port 0
                    ...
                    if (src_col != dst_col):  # Need to send horizontally
                        ...
                        if (diff >= 0):   # Send to the right, i.e. using out_port 3
                            ...
                            if options.dump_routing_file:
                                route_dump.write(
                                    'R'+str(src)+': '+str(dst)+' -> '+str(out_port)+'\n')
        ...
        for src in range(options.num_routers):
            ...
            for user_port in range(num_total_user_ports):
                ...
                if dst == src:  # packet is destined to me, extract from router, i.e. out_port 0
                    ...
                    if options.verbose:
                        print('route:'+str(src)+'->' +
                              str(user_port)+':'+str(out_port))
                    if options.dump_routing_file:
                        route_dump.write(
                            'R'+str(src)+': '+str(user_port)+' -> '+str(out_port)+'\n')
                else:           # packet is not for me, decide which way to send
                    src_row = src / options.routers_per_row
                    src_col = src % options.routers_per_row
                    dst_row = dst / options.routers_per_row
...
def find_dst_and_output_for_user_port(user_port):
    if (user_port < options.num_routers):
        ...
        for cur_rt in range(options.num_routers):
            ...
            is_bottom_side_router = cur_rt >= options.num_routers - \
                options.routers_per_row and cur_rt < options.num_routers
        ...
        for dst in range(options.num_routers):
            if src == dst:  # packet is destined to me, extract from router, i.e. out_port 0
                ...
                if (src_col != dst_col):  # Need to send horizontally
                    diff = dst_col-src_col
                    dist = abs(diff)
                    # Send to the right, i.e. using out_port 3
                    if (diff >= 0 and dist <= options.routers_per_row/2) or (diff < 0 and dist > options.routers_per_row/2):
                        out_port = 3
                        rt.write('%x\n' % (out_port))
                        if options.verbose:
                            print('route:'+str(src)+'->' +
                    ...
                    diff = dst_row-src_row
                    dist = abs(diff)
                    # Send down, i.e. using out_port 4
                    if (diff >= 0 and dist <= options.routers_per_column/2) or (diff < 0 and dist > options.routers_per_column/2):
                        out_port = 4
                        rt.write('%x\n' % (out_port))
                        if options.verbose:
                            print('route:'+str(src)+'->' +
    ...
    for s in range(0, options.num_routers):
        for d in range(0, options.num_routers):
            if (s != d):  # don't create a link to my self
                if (s < d):
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        s)+'], ' + str(d) + ', routers['+str(d)+'], ' + str(s+1) + ');\n')
                    if options.dump_topology_file:
                        topo_dump.write(
                            'RouterLink '+'R'+str(s)+':'+str(d)+' -> '+'R'+str(d)+':'+str(s+1)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(s)+' -> R'+str(d)+' [ taillabel = "' + str(
                            d) + '", headlabel = "' + str(s+1) + '" ];\n')
                else:
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        s)+'], ' + str(d+1) + ', routers['+str(d)+'], ' + str(s) + ');\n')
                    if options.dump_topology_file:
                        topo_dump.write(
                            'RouterLink '+'R'+str(s)+':'+str(d+1)+' -> '+'R'+str(d)+':'+str(s)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(s)+' -> R'+str(d)+' [ taillabel = "' + str(
                            d+1) + '", headlabel = "' + str(s) + '" ];\n')
        ...
        for dst in range(0, options.num_routers):
            if src == dst:  # packet is destined to me, extract from router, i.e. out_port 0
                ...
                if (src < dst):
                    rt.write('%x\n' % (dst))
                    if options.verbose:
                        print('route:'+str(src)+'->'+str(dst)+':'+str(dst))
                    if options.dump_routing_file:
                        route_dump.write('R'+str(src)+': ' +
                                         str(dst)+' -> '+str(dst)+'\n')
                else:
                    rt.write('%x\n' % (dst+1))
                    if options.verbose:
                        print('route:'+str(src)+'->'+str(dst)+':'+str(dst+1))
                    if options.dump_routing_file:
                        route_dump.write('R'+str(src)+': ' +
                                         str(dst)+' -> '+str(dst+1)+'\n')
...
def gen_fat_tree_links(links, dot_filename, dump_topology_filename):
    ...
    for r in range(options.num_routers):
        links.write(
            'send_ports_ifaces['+str(r)+'] = routers['+str(r/2)+'].in_ports['+str(r % 2)+'];\n')
        links.write(
            'recv_ports_ifaces['+str(r)+'] = routers['+str(r/2)+'].out_ports['+str(r % 2)+'];\n')
        if options.dump_topology_file:
            topo_dump.write('SendPort '+str(r)+' -> R' +
                            str(r/2)+':'+str(r % 2)+'\n')
            topo_dump.write('RecvPort '+str(r)+' -> R' +
                            str(r/2)+':'+str(r % 2)+'\n')
        if options.gen_graph and options.graph_nodes:  # also include the endpoints
            dot.write('N'+str(r)+' -> R'+str(r/2) +
                      ' [ headlabel = "' + str(r % 2) + '" ];\n')
            dot.write('R'+str(r/2)+' -> N'+str(r) +
                      ' [ taillabel = "' + str(r % 2) + '" ];\n')
        # links.write('router_info_ifaces['+str(r)+'] =  get_router_info_ifc('+str(r)+');\n')
        links.write(
            'recv_ports_info_ifaces['+str(r)+'] =  get_port_info_ifc('+str(r)+');\n')
    ...
    for stage in range(num_stages):
        if (stage < num_stages-1):  # not the top stage
            for sr in range(rts_in_stage):  # subrouter in particular stage
                ...
                # left subset, i.e. has stage-th bit set to 0  (connect above and to the right)
                if ((cur_rt & (1 << stage)) == 0):
                    # Up links
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt)+'], 2, routers['+str(cur_rt+rts_in_stage)+'], 0);\n')
                    if options.dump_topology_file:
                        topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(2) +
                                        ' -> '+'R'+str(cur_rt+rts_in_stage)+':'+str(0)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt)+' -> R'+str(cur_rt+rts_in_stage) +
                                  ' [ taillabel = "' + '2' + '", headlabel = "' + '0' + '" ];\n')
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt)+'], 3, routers['+str(cur_rt+rts_in_stage+(1 << stage))+'], 0);\n')
                    if options.dump_topology_file:
                        topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(3)+' -> ' +
                                        'R'+str(cur_rt+rts_in_stage+(1 << stage))+':'+str(0)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt)+' -> R'+str(cur_rt+rts_in_stage+(1 << stage)) +
                                  ' [ taillabel = "' + '3' + '", headlabel = "' + '0' + '" ];\n')

                    # Down links
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt+rts_in_stage)+'], 0, routers['+str(cur_rt)+'], 2);\n')
                    if options.dump_topology_file:
                        topo_dump.write('RouterLink '+'R'+str(cur_rt+rts_in_stage) +
                                        ':'+str(0)+' -> '+'R'+str(cur_rt)+':'+str(2)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt+rts_in_stage)+' -> R'+str(cur_rt) +
                                  ' [ taillabel = "' + '0' + '", headlabel = "' + '2' + '" ];\n')
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt+rts_in_stage+(1 << stage))+'], 0, routers['+str(cur_rt)+'], 3);\n')
                    if options.dump_topology_file:
                        topo_dump.write('RouterLink '+'R'+str(cur_rt+rts_in_stage+(
                            1 << stage))+':'+str(0)+' -> '+'R'+str(cur_rt)+':'+str(3)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt+rts_in_stage+(1 << stage))+' -> R'+str(
                            cur_rt)+' [ taillabel = "' + '0' + '", headlabel = "' + '3' + '" ];\n')

                # right subset, i.e. has stage-th bit set to 1  (connect to the left and above)
                else:
                    # Up links
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt)+'], 2, routers['+str(cur_rt+rts_in_stage-(1 << stage))+'], 1);\n')
                    if options.dump_topology_file:
                        topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(2)+' -> ' +
                                        'R'+str(cur_rt+rts_in_stage-(1 << stage))+':'+str(1)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt)+' -> R'+str(cur_rt+rts_in_stage-(1 << stage)) +
                                  ' [ taillabel = "' + '2' + '", headlabel = "' + '1' + '" ];\n')
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt)+'], 3, routers['+str(cur_rt+rts_in_stage)+'], 1);\n')
                    if options.dump_topology_file:
                        topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(3) +
                                        ' -> '+'R'+str(cur_rt+rts_in_stage)+':'+str(1)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt)+' -> R'+str(cur_rt+rts_in_stage) +
                                  ' [ taillabel = "' + '3' + '", headlabel = "' + '1' + '" ];\n')

                    # Down links
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt+rts_in_stage-(1 << stage))+'], 1, routers['+str(cur_rt)+'], 2);\n')
                    if options.dump_topology_file:
                        topo_dump.write('RouterLink '+'R'+str(cur_rt+rts_in_stage-(
                            1 << stage))+':'+str(1)+' -> '+'R'+str(cur_rt)+':'+str(2)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt+rts_in_stage-(1 << stage))+' -> R'+str(
                            cur_rt)+' [ taillabel = "' + '1' + '", headlabel = "' + '2' + '" ];\n')
                    link_id = link_id + 1
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt+rts_in_stage)+'], 1, routers['+str(cur_rt)+'], 3);\n')
                    if options.dump_topology_file:
                        topo_dump.write('RouterLink '+'R'+str(cur_rt+rts_in_stage) +
                                        ':'+str(1)+' -> '+'R'+str(cur_rt)+':'+str(3)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt+rts_in_stage)+' -> R'+str(cur_rt) +
                                  ' [ taillabel = "' + '1' + '", headlabel = "' + '3' + '" ];\n')
            ...
            for sr in range(rts_in_stage):
                ...
                # Up links
                link_id = link_id + 1
                links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(cur_rt) +
                            '], 2, routers['+str(first_top_rt+top_rt_offset)+'], '+str(top_port_id)+');\n')
                if options.dump_topology_file:
                    topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(2)+' -> ' +
                                    'R'+str(first_top_rt+top_rt_offset)+':'+str(top_port_id)+'\n')
                if options.gen_graph:
                    dot.write('R'+str(cur_rt)+' -> R'+str(first_top_rt+top_rt_offset) +
                              ' [ taillabel = "' + '2' + '", headlabel = "' + str(top_port_id) + '" ];\n')
                # Down links
                link_id = link_id + 1
                links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                    first_top_rt+top_rt_offset)+'], '+str(top_port_id)+', routers['+str(cur_rt)+'], 2);\n')
                if options.dump_topology_file:
                    topo_dump.write('RouterLink '+'R'+str(first_top_rt+top_rt_offset) +
                                    ':'+str(top_port_id)+' -> '+'R'+str(cur_rt)+':'+str(2)+'\n')
                if options.gen_graph:
                    dot.write('R'+str(first_top_rt+top_rt_offset)+' -> R'+str(cur_rt) +
                              ' [ taillabel = "' + str(top_port_id) + '", headlabel = "' + '2' + '" ];\n')
                top_rt_offset = (top_rt_offset + 1) % last_stages

                # Up Links
                link_id = link_id + 1
                links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(cur_rt) +
                            '], 3, routers['+str(first_top_rt+top_rt_offset)+'], '+str(top_port_id)+');\n')
                if options.dump_topology_file:
                    topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(3)+' -> ' +
                                    'R'+str(first_top_rt+top_rt_offset)+':'+str(top_port_id)+'\n')
                if options.gen_graph:
                    dot.write('R'+str(cur_rt)+' -> R'+str(first_top_rt+top_rt_offset) +
                              ' [ taillabel = "' + '3' + '", headlabel = "' + str(top_port_id) + '" ];\n')
                # Down links
                link_id = link_id + 1
                links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                    first_top_rt+top_rt_offset)+'], '+str(top_port_id)+', routers['+str(cur_rt)+'], 3);\n')
                if options.dump_topology_file:
                    topo_dump.write('RouterLink '+'R'+str(first_top_rt+top_rt_offset) +
                                    ':'+str(top_port_id)+' -> '+'R'+str(cur_rt)+':'+str(3)+'\n')
                if options.gen_graph:
                    dot.write('R'+str(first_top_rt+top_rt_offset)+' -> R'+str(cur_rt) +
                              ' [ taillabel = "' + str(top_port_id) + '", headlabel = "' + '3' + '" ];\n')
                top_rt_offset = (top_rt_offset + 1) % last_stages
                ...
                for dst in range(options.num_routers):
                    ...
                    if (dst >> stage+1 == sr >> stage):  # go down
                        ...
                        if (dst & (1 << (stage)) == 0):   # go left
                            rt.write('%x\n' % (0))
                            if options.verbose:
                                print('route:'+str(cur_rt) +
                                      '->'+str(dst)+':'+'0')
                            if options.dump_routing_file:
                                route_dump.write(
                                    'R'+str(cur_rt)+': '+str(dst)+' -> '+str(0)+'\n')
                        else:  # go right
                            rt.write('%x\n' % (1))
                            if options.verbose:
                                print('route:'+str(cur_rt) +
                ...
                for dst in range(options.num_routers):
                    ...
                    if options.verbose:
                        print('route:'+str(cur_rt)+'->' +
                              str(dst)+':'+str(out_port))
                    if options.dump_routing_file:
                        route_dump.write('R'+str(cur_rt)+': ' +
                                         str(dst)+' -> '+str(out_port)+'\n')
                rt.close()
                if options.verbose:
                    print('Generated routing file: ' + filename)
    # Hack - set num_routers to actual number of routers
    actual_routers = (num_stages-1) * (options.num_routers/2) + \
        (options.num_routers/4)  # n-1 stages + last stage
...
def gen_butterfly_links(links, dot_filename, dump_topology_filename):
    ...
    for r in range(options.num_routers):
        links.write(
            'send_ports_ifaces['+str(r)+'] = routers['+str(r/2)+'].in_ports['+str(r % 2)+'];\n')
        links.write('recv_ports_ifaces['+str(r)+'] = routers['+str(
            last_stage_offset + r/2)+'].out_ports['+str(r % 2)+'];\n')
        if options.dump_topology_file:
            topo_dump.write('SendPort '+str(r)+' -> R' +
                            str(r/2)+':'+str(r % 2)+'\n')
            topo_dump.write('RecvPort '+str(r)+' -> R' +
                            str(last_stage_offset + r/2)+':'+str(r % 2)+'\n')
        if options.gen_graph and options.graph_nodes:  # also include the endpoints
            dot.write('N'+str(r+options.num_routers)+' -> R' +
                      str(r/2)+' [ headlabel = "' + str(r % 2) + '" ];\n')
            dot.write('R'+str(last_stage_offset + r/2)+' -> N' +
                      str(r)+' [ taillabel = "' + str(r % 2) + '" ];\n')
        # links.write('router_info_ifaces['+str(r)+'] =  get_router_info_ifc('+str(r)+');\n')
        links.write(
            'recv_ports_info_ifaces['+str(r)+'] =  get_port_info_ifc('+str(r)+');\n')
    ...
    for stage in range(link_stages):  # first stage is the most-right one
        for sr in range(rts_in_stage):  # subrouter in particular stage
            ...
            # top subset  (connect to the right and to bottom right)
            if ((cur_rt & 1 << rev_stage) == 0):
                # Right links
                link_id = link_id + 1
                links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                            str(cur_rt)+'], 0, routers['+str(cur_rt+rts_in_stage)+'], 0);\n')
                if options.dump_topology_file:
                    topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(0) +
                                    ' -> '+'R'+str(cur_rt+rts_in_stage)+':'+str(0)+'\n')
                if options.gen_graph:
                    dot.write('R'+str(cur_rt)+' -> R'+str(cur_rt+rts_in_stage) +
                              ' [ taillabel = "' + '0' + '", headlabel = "' + '0' + '" ];\n')
                link_id = link_id + 1
                links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                    cur_rt)+'], 1, routers['+str(cur_rt+rts_in_stage+(1 << rev_stage))+'], 0);\n')
                if options.dump_topology_file:
                    topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(1)+' -> ' +
                                    'R'+str(cur_rt+rts_in_stage+(1 << rev_stage))+':'+str(0)+'\n')
                if options.gen_graph:
                    dot.write('R'+str(cur_rt)+' -> R'+str(cur_rt+rts_in_stage+(1 << rev_stage)
                                                          )+' [ taillabel = "' + '1' + '", headlabel = "' + '0' + '" ];\n')
                ...
                link_id = link_id + 1
                links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                    cur_rt)+'], 0, routers['+str(cur_rt+rts_in_stage-(1 << rev_stage))+'], 1);\n')
                if options.dump_topology_file:
                    topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(0)+' -> ' +
                                    'R'+str(cur_rt+rts_in_stage-(1 << rev_stage))+':'+str(1)+'\n')
                if options.gen_graph:
                    dot.write('R'+str(cur_rt)+' -> R'+str(cur_rt+rts_in_stage-(1 << rev_stage)
                                                          )+' [ taillabel = "' + '0' + '", headlabel = "' + '1' + '" ];\n')
                link_id = link_id + 1
                links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                            str(cur_rt)+'], 1, routers['+str(cur_rt+rts_in_stage)+'], 1);\n')
                if options.dump_topology_file:
                    topo_dump.write('RouterLink '+'R'+str(cur_rt)+':'+str(1) +
                                    ' -> '+'R'+str(cur_rt+rts_in_stage)+':'+str(1)+'\n')
                if options.gen_graph:
        ...
        for sr in range(rts_in_stage):  # subrouter in particular stage
            ...
            for dst in range(options.num_routers):
                ...
                if (dst & (1 << (rev_stage)) == 0):   # go down
                    rt.write('%x\n' % (0))
                    if options.verbose:
                        print('route:'+str(cur_rt)+'->'+str(dst)+':'+'0')
                    if options.dump_routing_file:
                        route_dump.write('R'+str(cur_rt)+': ' +
                                         str(dst)+' -> '+str(0)+'\n')
                else:  # go up
                    rt.write('%x\n' % (1))
                    if options.verbose:
                        print('route:'+str(cur_rt)+'->'+str(dst)+':'+'1')
                    if options.dump_routing_file:
                        route_dump.write('R'+str(cur_rt)+': ' +
                                         str(dst)+' -> '+str(1)+'\n')
            ...
            if options.verbose:
                print('Generated routing file: ' + filename)
    # Hack - set num_routers to actual number of routers
    actual_routers = (num_stages) * (options.num_routers /
                                     2)  # n stages, each n/2 routers
    options.num_routers = actual_routers
...
# Expose user send/receive ports
    ...
    for r in range(options.send_endpoints):
        ...
        if options.gen_graph and options.graph_nodes:  # also include the endpoints
            ...
            dot.write('N'+str(r+options.recv_endpoints)+' -> R' +
                      str(0)+' [ headlabel = "' + str(r) + '" ];\n')
        # links.write('router_info_ifaces['+str(r)+'] =  get_router_info_ifc('+str(r)+');\n')
...
def gen_uni_tree_links(links, dot_filename, dump_topology_filename):
    ...
    if (build_down_tree):  # Build down tree
        ...
        for i in range(options.uni_tree_inputs):
            links.write(
                'send_ports_ifaces['+str(i)+'] = routers[0].in_ports['+str(i)+'];\n')
            if options.dump_topology_file:
                topo_dump.write('SendPort '+str(i)+' -> R' +
                                str(0)+':'+str(i)+'\n')
            if options.gen_graph and options.graph_nodes:  # also include the endpoints
                dot.write('N'+str(i+options.uni_tree_outputs)+' -> R' +
                          str(0)+' [ headlabel = "' + str(i) + '" ];\n')
        # Find first and last router id of last stage where nodes attach
        final_stage_rt_first_id = 0
        for i in range(num_stages):
            final_stage_rt_first_id += options.uni_tree_fanout**i
        ...
        num_final_stage_rts = (final_stage_rt_last_id+1) - \
            final_stage_rt_first_id
        if (options.uni_tree_distribute_leaves):
            for recv_port_id in range(options.uni_tree_outputs):
                o = final_stage_rt_first_id + \
                    get_uni_tree_distributed_rt(
                        recv_port_id, num_stages, options.uni_tree_fanout)
                p = recv_port_id/(options.uni_tree_fanout**num_stages)
                links.write(
                    'recv_ports_ifaces['+str(recv_port_id)+'] = routers['+str(o)+'].out_ports['+str(p)+'];\n')
                if options.dump_topology_file:
                    topo_dump.write('RecvPort '+str(recv_port_id) +
                                    ' -> R'+str(0)+':'+str(p)+'\n')
                if options.gen_graph and options.graph_nodes:  # also include the endpoints
                    dot.write('R'+str(o)+' -> N'+str(recv_port_id) +
                              ' [ taillabel = "' + str(p) + '" ];\n')
                links.write(
                    'recv_ports_info_ifaces['+str(recv_port_id)+'] =  get_port_info_ifc('+str(recv_port_id)+');\n')
                recv_port_id += 1
        else:
            recv_port_id = 0
            for o in range(final_stage_rt_first_id, final_stage_rt_last_id+1):
                for p in range(options.uni_tree_fanout):
                    # Stop once you've created enough output ports, even if the tree has more outputs
                    if (recv_port_id < options.uni_tree_outputs):
                        links.write(
                            'recv_ports_ifaces['+str(recv_port_id)+'] = routers['+str(o)+'].out_ports['+str(p)+'];\n')
                        if options.dump_topology_file:
                            topo_dump.write(
                                'RecvPort '+str(recv_port_id)+' -> R'+str(0)+':'+str(p)+'\n')
                        if options.gen_graph and options.graph_nodes:  # also include the endpoints
                            dot.write('R'+str(o)+' -> N'+str(recv_port_id) +
                                      ' [ taillabel = "' + str(p) + '" ];\n')
                        links.write(
        ...
        for stage in range(num_stages):
            ...
            for r in range(rts_in_stage):
                for l in range(options.uni_tree_fanout):
                    ...
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        cur_rt)+'], '+str(l)+', routers['+str(dest_rt)+'], 0);\n')
                    if options.dump_topology_file:
                        topo_dump.write(
                            'RouterLink '+'R'+str(cur_rt)+':'+str(l)+' -> '+'R'+str(dest_rt)+':'+str(0)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(cur_rt)+' -> R'+str(dest_rt) +
                                  ' [ taillabel = "' + str(l) + '", headlabel = "' + '0' + '" ];\n')
                cur_rt += 1
            rts_in_stage *= options.uni_tree_fanout
        ...
        for o in range(options.uni_tree_outputs):
            links.write(
                'recv_ports_ifaces['+str(o)+'] = routers[0].out_ports['+str(o)+'];\n')
            if options.dump_topology_file:
                topo_dump.write('RecvPort '+str(o)+' -> R' +
                                str(0)+':'+str(o)+'\n')
            if options.gen_graph and options.graph_nodes:  # also include the endpoints
                dot.write('R'+str(0)+' -> N'+str(o) +
                          ' [ taillabel = "' + str(o) + '" ];\n')
            links.write(
                'recv_ports_info_ifaces['+str(o)+'] =  get_port_info_ifc('+str(o)+');\n')
        # Find first and last router id of last stage where nodes attach
        final_stage_rt_first_id = 0
        for i in range(num_stages):
            final_stage_rt_first_id += options.uni_tree_fanout**i
        ...
        if (options.uni_tree_distribute_leaves):
            for send_port_id in range(options.uni_tree_inputs):
                o = final_stage_rt_first_id + \
                    get_uni_tree_distributed_rt(
                        send_port_id, num_stages, options.uni_tree_fanout)
                p = send_port_id/(options.uni_tree_fanout**num_stages)
                links.write(
                    'send_ports_ifaces['+str(send_port_id)+'] = routers['+str(o)+'].in_ports['+str(p)+'];\n')
                if options.dump_topology_file:
                    topo_dump.write('SendPort '+str(send_port_id) +
                                    ' -> R'+str(o)+':'+str(p)+'\n')
                if options.gen_graph and options.graph_nodes:  # also include the endpoints
                    dot.write('N'+str(send_port_id+options.uni_tree_outputs) +
                              ' -> R'+str(o)+' [ headlabel = "' + str(p) + '" ];\n')
                send_port_id += 1
        else:
            send_port_id = 0
            for o in range(final_stage_rt_first_id, final_stage_rt_last_id+1):
                for p in range(options.uni_tree_fanout):
                    # Stop once you've created enough input ports, even if the tree has more outputs
                    if (send_port_id < options.uni_tree_inputs):
                        links.write(
                            'send_ports_ifaces['+str(send_port_id)+'] = routers['+str(o)+'].in_ports['+str(p)+'];\n')
                        if options.dump_topology_file:
                            topo_dump.write(
                                'SendPort '+str(send_port_id)+' -> R'+str(o)+':'+str(p)+'\n')
                        if options.gen_graph and options.graph_nodes:  # also include the endpoints
                            dot.write('N'+str(send_port_id+options.uni_tree_outputs) +
                                      ' -> R'+str(o)+' [ headlabel = "' + str(p) + '" ];\n')
                        send_port_id += 1
        ...
        for stage in range(num_stages):
            for r in range(rts_in_stage):
                for l in range(options.uni_tree_fanout):
                    ...
                    links.write('links['+str(link_id)+'] <- mkConnectPorts(routers['+str(
                        src_rt)+'], 0, routers['+str(cur_rt)+'], '+str(l)+');\n')
                    if options.dump_topology_file:
                        topo_dump.write(
                            'RouterLink '+'R'+str(src_rt)+':'+str(0)+' -> '+'R'+str(cur_rt)+':'+str(l)+'\n')
                    if options.gen_graph:
                        dot.write('R'+str(src_rt)+' -> R'+str(cur_rt) +
                                  ' [ taillabel = "' + '0' + '", headlabel = "' + str(l) + '" ];\n')
                cur_rt += 1
            rts_in_stage *= options.uni_tree_fanout
        ...
        for stage in range(num_stages+1):
            for r in range(rts_in_stage):
                ...
                for dst in range(options.uni_tree_outputs):
                    if (options.uni_tree_distribute_leaves):  # distribute leaves
                        out_port = dst / \
                            (options.uni_tree_fanout**(stage)
                             ) % options.uni_tree_fanout
                    else:
                        out_port = dst / \
                            (options.uni_tree_fanout**(num_stages-stage)
                             ) % options.uni_tree_fanout
                    rt.write('%x\n' % out_port)
                    if options.verbose:
                        print('route:'+str(cur_rt)+'->' +
...
def parse_custom_topology(custom_topology_file, send_ports, recv_ports, router_links, topology_info):
    ...
    try:
        c_topology = open(options.custom_topology, 'r')
    except IOError:
        print("Could not open custom topology file " + options.custom_topology)
        sys.exit(-1)
    ...
    for i, l in enumerate(c_topology):
        ...
        if (m is not None):
            ...
            if options.verbose:
                print('Attaching Send Port %s to R%s:%s' %
                      (send_port, rt, port))
            ...
            if (topology_info['num_routers'] < (int(rt)+1)):
                # update total number of routers
                topology_info['num_routers'] = int(rt)+1
            if (topology_info['max_num_in_ports'] < (int(port)+1)):
                topology_info['max_num_in_ports'] = int(
                    port)+1  # update max number of input ports
            if (topology_info['max_send_ports'] < (int(send_port)+1)):
                topology_info['max_send_ports'] = int(
                    send_port)+1  # update max number of send ports
            continue
        ...
        if (m is not None):
            ...
            if options.verbose:
                print('Attaching Recv Port %s to R%s:%s' %
                      (recv_port, rt, port))
            ...
            if (topology_info['num_routers'] < (int(rt)+1)):
                # update total number of routers
                topology_info['num_routers'] = int(rt)+1
            if (topology_info['max_num_out_ports'] < (int(port)+1)):
                topology_info['max_num_out_ports'] = int(
                    port)+1  # update max number of input ports
            if (topology_info['max_recv_ports'] < (int(recv_port)+1)):
                topology_info['max_recv_ports'] = int(
        ...
        if (m is not None):
            ...
            if (topology_info['max_num_in_ports'] < (int(port)+1)):
                topology_info['max_num_in_ports'] = int(
                    port)+1  # update max number of input ports
            if (topology_info['max_send_ports'] < (int(user_port)+1)):
                topology_info['max_send_ports'] = int(
                    user_port)+1  # update max number of send ports
            if (topology_info['max_recv_ports'] < (int(user_port)+1)):
                topology_info['max_recv_ports'] = int(
                    user_port)+1  # update max number of send ports
            continue
        m = re.match(
        ...
        if (m is not None):
            ...
            if options.verbose:
                print('Connecting R%s:%s to R%s:%s' %
                      (src_rt, src_port, dst_rt, dst_port))
            if (src_rt not in router_links):
                router_links[src_rt] = dict()
            ...
            if (src_port in router_links[src_rt]):  # Overwriting connections
                ...
                print('Warning: R%s:%s was already connected to R%s:%s, but will now be connected to R%s:R%s' % (
                    src_rt, src_port, exist_dest[0], exist_dest[1], dst_rt, dst_port))
            ...
            # Update topology info
            if (topology_info['num_routers'] < (int(src_rt)+1)):
                # update total number of routers
                topology_info['num_routers'] = int(src_rt)+1
            if (topology_info['num_routers'] < (int(dst_rt)+1)):
                # update total number of routers
                topology_info['num_routers'] = int(dst_rt)+1
            if (topology_info['max_num_out_ports'] < (int(src_port)+1)):
                topology_info['max_num_out_ports'] = int(
                    src_port)+1  # update max number of input ports
            if (topology_info['max_num_in_ports'] < (int(dst_port)+1)):
                topology_info['max_num_in_ports'] = int(
                    dst_port)+1  # update max number of input ports

            # For sanity check
            if (dst_rt not in reverse_router_links):
                reverse_router_links[dst_rt] = dict()
            reverse_router_links[dst_rt][dst_port] = [src_rt, src_port]

            continue
        print('Error parsing custom_topology_file (%s) at line %d: "%s"' %
              (custom_topology_file, i, l[:-1]))
        sys.exit(-1)

    # Sanity checks
    for r in range(topology_info['num_routers']):
        # A router that is unconnected
        if ((str(r) not in router_links) and (str(r) not in reverse_router_links)):
            print(
                'Error: Missing topology info for router %d. Router IDs must be consecutive.' % (r))
            sys.exit(-1)

    for s in range(topology_info['max_send_ports']):
        if (str(s) not in send_ports):  # A non-existent send port
            print(
                'Error: Missing topology info for send port %d. Send port IDs must be consecutive.' % (s))
            sys.exit(-1)

    for r in range(topology_info['max_recv_ports']):
        if (str(r) not in recv_ports):  # A non-existent recv port
            print(
                'Error: Missing topology info for recv port %d. Recv port IDs must be consecutive.' % (r))
            sys.exit(-1)
    ...
    if options.verbose:
        print('Total number of routers:', topology_info['num_routers'])
    if options.verbose:
        print('Max number of in ports:', topology_info['max_num_in_ports'])
    if options.verbose:
        print('Max number of out ports:', topology_info['max_num_out_ports'])
    if options.verbose:
        print('Max number of send ports:', topology_info['max_send_ports'])
    if options.verbose:
        print('Max number of recv ports:', topology_info['max_recv_ports'])
...
def parse_custom_routing(custom_routing_file, topology_info):
    ...
    global options, args
    try:
        c_routing = open(custom_routing_file, 'r')
    except IOError:
        print("Could not open custom routing file " + options.custom_routing)
        sys.exit(-1)
    ...
    for i, l in enumerate(c_routing):
        ...
        if (m is not None):
            ...
            if (int(rt) >= topology_info['num_routers']):
                print(
                    'Routing entry for non-existent router %s at line %d: "%s"' % (rt, i, l[:-1]))
                sys.exit(-1)
            if (int(dst) >= topology_info['max_dests']):
                print('Routing entry for router %s has non-existent destination (%s) at line %d: "%s"' %
                      (rt, dst, i, l[:-1]))
                sys.exit(-1)
            if (int(port) >= topology_info['max_num_out_ports']):
                print('Routing entry for router %s has non-existent out port (%s) at line %d: "%s"' %
                      (rt, port, i, l[:-1]))
                sys.exit(-1)

            if (routing[int(rt)][int(dst)] != -1):
                print('Warning: Overwriting previous routing entry (R%s:%s->%d) with new routing entry (R%s:%s->%s)' %
                      (rt, dst, routing[int(rt)][int(dst)], rt, dst, port))
            # if options.verbose: print 'Attaching Send Port %s to R%s:%s' % (m.group(1), m.group(2), m.group(3))
            if options.verbose:
                print('New routing entry for router %s: dst:%s -> out_port:%s' %
                      (rt, dst, port))
            routing[int(rt)][int(dst)] = int(port)

            continue
        print('Error parsing routing_topology_file (%s) at line %d: "%s"' %
              (routing_topology_file, i, l[:-1]))
        sys.exit(-1)
...
def gen_custom_links(send_ports, recv_ports, router_links, topology_info, links, dot_filename):
    ...
    # Generate graphviz .gv file
    if options.gen_graph:
        dot = prepare_graph_file(dot_filename + ".gv", "circo", topology_info)

# Expose user send/receive and info ports
    for s, rp in list(send_ports.items()):
        links.write(
            'send_ports_ifaces['+s+'] = routers['+rp[0]+'].in_ports['+rp[1]+'];\n')
        if options.graph_nodes:
            dot.write('N'+s+' -> R'+rp[0] +
                      ' [ headlabel = "' + rp[1] + '" ];\n')
    for r, rp in list(recv_ports.items()):
        links.write(
            'recv_ports_ifaces['+r+'] = routers['+rp[0]+'].out_ports['+rp[1]+'];\n')
        if options.graph_nodes:
            dot.write('R'+rp[0]+' -> N'+r +
                      ' [ taillabel = "' + rp[1] + '" ];\n')
        links.write(
            'recv_ports_info_ifaces['+r+'] =  get_port_info_ifc('+r+');\n')
    ...
    for src_rt in list(router_links.keys()):
        for src_port in list(router_links[src_rt].keys()):
            ...
            dst_port = tmp[1]
            links.write('links['+str(link_id)+'] <- mkConnectPorts(routers[' +
                        src_rt+'], '+src_port+', routers['+dst_rt+'], '+dst_port+');\n')
            if options.gen_graph:
                dot.write('R'+src_rt+' -> R'+dst_rt +
                          ' [ taillabel = "' + src_port + '", headlabel = "' + dst_port + '" ];\n')
            link_id = link_id + 1
...
def gen_net_parameters(num_user_send_ports, num_user_recv_ports, num_in_ports, num_out_ports, num_links, cut, filename='network_parameters.bsv'):
    ...
    parameters.write('`define NUM_USER_SEND_PORTS ' +
                     str(num_user_send_ports) + '\n')
    parameters.write('`define NUM_USER_RECV_PORTS ' +
                     str(num_user_recv_ports) + '\n')
    ...
    parameters.write('`define USE_VIRTUAL_LINKS ' +
                     str(options.use_virtual_links) + '\n')
    parameters.write('`define FLIT_BUFFER_DEPTH ' +
                     str(options.flit_buffer_depth) + '\n')
    parameters.write('`define FLIT_DATA_WIDTH ' +
                     str(options.flit_data_width) + '\n')
    ...
    if options.verbose:
        print('Generated parameters file: ' + filename)

def gen_cdc_top_bsv(num_send_ports, num_recv_ports, depth, filename='NetworkCDCTop.bsv'):
    global options, args
    try:
        f = open(filename, 'w')
    except IOError:
    ...
    f.write('import Clocks::*;\n')
    f.write('import Network::*;\n')
    f.write('import NetworkSimple::*;\n')
    f.write('import NetworkCDC::*;\n')
    f.write('import NetworkTypes::*;\n\n')
    
    if (options.flow_control_type == "peek"):
        f.write('(* synthesize *)\n')
        f.write('module mkNetworkSimpleCDCTop(\n')
        for p in range(num_send_ports):
            f.write('    Clock pe_send_clk_%d, Reset pe_send_rst_%d,\n' % (p, p))
        for p in range(num_recv_ports):
            f.write('    Clock pe_recv_clk_%d, Reset pe_recv_rst_%d,\n' % (p, p))
        f.write('    NetworkSimple ifc);\n\n')
        
        f.write('    Vector#(NumUserSendPorts, Clock) send_clks = newVector();\n')
        f.write('    Vector#(NumUserSendPorts, Reset) send_rsts = newVector();\n')
        for p in range(num_send_ports):
            f.write('    send_clks[%d] = pe_send_clk_%d;\n' % (p, p))
            f.write('    send_rsts[%d] = pe_send_rst_%d;\n' % (p, p))
            
        f.write('    Vector#(NumUserRecvPorts, Clock) recv_clks = newVector();\n')
        f.write('    Vector#(NumUserRecvPorts, Reset) recv_rsts = newVector();\n')
        for p in range(num_recv_ports):
            f.write('    recv_clks[%d] = pe_recv_clk_%d;\n' % (p, p))
            f.write('    recv_rsts[%d] = pe_recv_rst_%d;\n' % (p, p))
            
        f.write('    let _noc <- mkNetworkSimpleCDCWrapper(%d, send_clks, send_rsts, recv_clks, recv_rsts);\n' % depth)
        f.write('    return _noc;\n')
        f.write('endmodule\n')
    else:
        f.write('(* synthesize *)\n')
        f.write('module mkNetworkCDCTop(\n')
        for p in range(num_send_ports):
            f.write('    Clock pe_send_clk_%d, Reset pe_send_rst_%d,\n' % (p, p))
        for p in range(num_recv_ports):
            f.write('    Clock pe_recv_clk_%d, Reset pe_recv_rst_%d,\n' % (p, p))
        ...
        f.write('    let _noc <- mkNetworkCDCWrapper(%d, send_clks, send_rsts, recv_clks, recv_rsts);\n' % depth)
        f.write('    return _noc;\n')
        f.write('endmodule\n')
    
    f.close()
    if options.verbose:
        print('Generated CDC top file: ' + filename)
...
def main():
    ...
    if (options.router_type == "voq"):
        if (not (options.flow_control_type == "peek")):
            print(
                'Warning: VOQ routers require the use of "peek" flow control. Setting flow control to "peek".')
            options.flow_control_type = True
        if (options.num_vcs > 1):
            print(
                'Warning: VOQ routers do not support multiple Virtual Channels (VCs). Setting num_vcs to 1.')
            options.num_vcs = 1

    else:  # VC-based routers
        if (options.num_vcs == 1):
            print(
                'Warning: VC-based routers require at least two Virtual Channels (VCs). Setting num_vcs to 2.')
            options.num_vcs = 2

    # Prefix for generated files (some topologies might add more fields to this - custom overrides this)
    file_prefix = ''
    if (options.file_prefix == ''):
        file_prefix = options.topology + '_' + str(options.num_routers) + 'RTs_' + str(options.num_vcs) + 'VCs_' + str(
            options.flit_buffer_depth) + 'BD_' + str(options.flit_data_width) + 'DW_' + str(options.alloc_type) + 'Alloc'
    else:
    ...
    #### star ####
        ...
        if (options.num_routers > 16):
            print('Error:', options.topology,
                  'topology only supports up to 16 routers.')
            sys.exit(1)
        ...
        # Set topology-specific parameters here
        # central node is connected to num_routers-1 other routers, plus local port
        max_num_in_ports = options.num_routers-1
        # central node is connected to num_routers-1 other routers, plus local port
        max_num_out_ports = options.num_routers-1
    ...
    #### mesh ####
        ...
        if (num_routers_is_square and options.routers_per_row == -1 and options.routers_per_column == -1):
            if options.verbose:
                print('num_routers(%d) is perfect square; setting options.routers_per_row and options.routers_per_column to sqrt(num_routers) to build square mesh.' % (
                    options.num_routers))
        ...
        if (options.routers_per_row * options.routers_per_column != options.num_routers):
            print('Error:', options.topology, 'topology requires that routers_per_row(%d) and routers_per_column(%d) are specified and that their product is equal to num_routers(%d).' % (
                options.routers_per_row, options.routers_per_column, options.num_routers))
            sys.exit(1)
        ...
        if (options.file_prefix == ''):
            file_prefix = file_prefix + '_' + \
                str(options.routers_per_row) + 'RTsPerRow_' + \
                str(options.routers_per_column) + 'RTsPerCol'
    ...
    #### torus ####
        ...
        if (num_routers_is_square and options.routers_per_row == -1 and options.routers_per_column == -1):
            if options.verbose:
                print('num_routers(%d) is perfect square; setting options.routers_per_row and options.routers_per_column to sqrt(num_routers) to build square torus.' % (
                    options.num_routers))
            options.routers_per_row = num_routers_sqrt
            options.routers_per_column = num_routers_sqrt
    ...
    #### fat tree ####
        ...
        num_is_power_of_two = options.num_routers != 0 and (
            (options.num_routers & (options.num_routers - 1)) == 0)
        num_routers_sqrt = (int)(math.floor(math.sqrt(options.num_routers)))
        num_routers_is_square = (options.num_routers == num_routers_sqrt**2)
        if (not num_is_power_of_two):
            print('Error:', options.topology,
                  'topology requires that num_routers is a power of two.')
            sys.exit(1)
    ...
    #### butterfly ####
        ...
        # Set topology-specific parameters here
        # only 2-ary butterfly topology based on 2x2 routers supported for now
        max_num_in_ports = 2
        # only 2-ary butterfly topology based on 2x2 routers supported for now
        max_num_out_ports = 2
    ...
    #### aggregation uni-directional tree ####
        ...
        if (num_leaf_nodes > 512):
            print('Error', options.topology,
                  'topology only supports up to 512 leaf nodes.')
            sys.exit(1)
        ...
        if (options.uni_tree_fanout < 2):  # automatically set fanout, default fan_out is 4
            if (num_leaf_nodes <= 4):
                ...
                options.uni_tree_fanout = int(
                    math.ceil(math.pow(num_leaf_nodes, 1.0/2.0)))
            elif (num_leaf_nodes <= 256):
                options.uni_tree_fanout = int(
                    math.ceil(math.pow(num_leaf_nodes, 1.0/3.0)))
            print('Warning', options.topology,
                  'topology requires that uni_tree_fanout is at least two. Setting uni_tree_fanout to ', options.uni_tree_fanout)
        ...
        if (options.file_prefix == ''):
            file_prefix = options.topology + '_' + str(options.uni_tree_inputs) + 'INs_' + str(options.uni_tree_outputs) + 'OUTs_' + str(options.uni_tree_fanout) + 'FANOUT_' + str(
                options.num_vcs) + 'VCs_' + str(options.flit_buffer_depth) + 'BD_' + str(options.flit_data_width) + 'DW_' + str(options.alloc_type) + 'Alloc'
        # Set topology-specific parameters here
        num_user_send_ports = options.uni_tree_inputs
        num_user_recv_ports = options.uni_tree_outputs
...
# sys.exit(1)
    ...
    #### Custom ####
        ...
        if (options.custom_topology == "" or options.custom_routing == ""):
            print("You must specify a custom topology and custom routing file using the --custom_topology and --custom_routing command-line options!")
            sys.exit(-1)

        try:
            c_topology = open(options.custom_topology, 'r')
        except IOError:
            print("Could not open custom topology file " +
                  options.custom_topology)
            sys.exit(-1)
        try:
            c_routing = open(options.custom_routing, 'r')
        except IOError:
            print("Could not open custom routing file " + options.custom_routing)
            sys.exit(-1)
        ...
        if (options.file_prefix == ''):
            file_prefix = options.topology + hash + '_' + str(options.num_routers) + 'RTs_' + str(options.num_vcs) + 'VCs_' + str(
                options.flit_buffer_depth) + 'BD_' + str(options.flit_data_width) + 'DW_' + str(options.alloc_type) + 'Alloc'

        # Parse custom topology and routing files - might change options.num_routers
        send_ports = dict()
        recv_ports = dict()
        router_links = dict()  # will get populated by parse_custom_topology
        topology_info = {'num_routers': 0, 'max_num_in_ports': 0,
                         'max_num_out_ports': 0, 'max_send_ports': 0, 'max_recv_ports': 0}
        parse_custom_topology(
            options.custom_topology, send_ports, recv_ports, router_links, topology_info)
...
#      if options.verbose: print 'Custom topology info\n  Number of routers: %d\n  Max input ports per router: %d\n  Max output ports per router: %d\n  Number of user send ports: %d\n  Number of user receive ports: %d\n' % (num_total_user_ports)
    ...
    try:
        ...
        print("Could not open file " + network_links_filename)
        sys.exit(-1)
    ...
    if (options.topology != "custom"):
        num_links = eval(generate_links_function)(
            links, dot_filename, dump_topology_filename)
        eval(generate_routing_function)(
            network_routing_file_prefix, dump_routing_filename)
    else:
        num_links = gen_custom_links(
            send_ports, recv_ports, router_links, topology_info, links, dot_filename)
        gen_custom_routing(routing, topology_info, network_routing_file_prefix)

    links.close()  # close links file
    if options.verbose:
        print('Generated links file: ' + network_links_filename)

    gen_net_parameters(num_user_send_ports, num_user_recv_ports, max_num_in_ports,
                       max_num_out_ports, num_links, options.cut, network_parameters_file)
    print('Generated ' + options.topology +
          ' network configuration succesfully.')
    ...
    if options.gen_graph:
        ...
        command = 'dot -T'+options.graph_format + ' -K' + options.graph_layout + ' ' + options.output_dir + \
            '/' + file_prefix+'_links.bsv.gv -o ' + options.output_dir + \
            '/' + file_prefix+'.'+options.graph_format
        os.system(command)
        if options.verbose:
            print('Generated network graph visuzalization file: ' +
                  file_prefix+'.'+options.graph_format)
    ...
    if (options.gen_rtl):
        ...
        if options.topology == 'xbar':
            xtra_flags = "-D XBAR=1 -D XBAR_LANES=" + \
                str(options.xbar_lanes) + " "

        user_flags = 'USER_FLAGS=\'' + xtra_flags + '-D NETWORK_PARAMETERS_FILE="\\"' + network_parameters_file + \
            '"\\" -D NETWORK_LINKS_FILE="\\"' + network_links_file + \
            '"\\" -D NETWORK_ROUTING_FILE_PREFIX="\\"' + \
            network_routing_file_prefix + '"\\"\''
        ...
        if (options.cdc):
            gen_cdc_top_bsv(num_user_send_ports, num_user_recv_ports, options.cdc_depth, "NetworkCDCTop.bsv")
            command = 'make net_cdc_top ' + user_flags
            if (options.flow_control_type == "peek"):
                command = 'make net_simple_cdc_top ' + user_flags
    ...
    if (options.run_xst):
        user_flags = 'USER_FLAGS=\'-D NETWORK_PARAMETERS_FILE="\\"' + network_parameters_file + '"\\" -D NETWORK_LINKS_FILE="\\"' + \
            network_links_file + '"\\" -D NETWORK_ROUTING_FILE_PREFIX="\\"' + \
            network_routing_file_prefix + '"\\"\''
...
if __name__ == '__main__':
    try:
        ...
        ), usage=globals()['__doc__'], version='0.6')
        ...
        parser.add_option('--sendmail', action='store', type="string",
                          default='', help='Send email notification when done')
        parser.add_option('-n', '--num_routers', action='store', type="int",
                          default=4, help='Specifies number of endpoint routers')
        parser.add_option('--send_endpoints', action='store', type="int", default=4,
                          help='Specifies number of send endpoints for uni-directional topologies')
        parser.add_option('--recv_endpoints', action='store', type="int", default=4,
                          help='Specifies number of receive endpoints for uni-directional topologies')
        ...
        parser.add_option('-v', '--num_vcs', action='store', type="int",
                          default=2, help='specifies number of virtual channels')
        ...
        parser.add_option('--use_virtual_links', action='store_true', default=False,
                          help='Enables locking of virtual links (VC+OutPort) in the presence of multi-flit packets.')
        ...
        parser.add_option('-d', '--flit_buffer_depth', action='store',
                          type="int", default=4, help='specifies depth of flit buffers')
        ...
        parser.add_option('-w', '--flit_data_width', action='store',
                          type="int", default=256, help='specifies flit data width')
        parser.add_option('-i', '--cut', action='store', type="int",
                          default=0, help='specifies the cut in an ideal or xbar network')
        parser.add_option('-p', '--file_prefix', action='store',
                          type="string", default='', help='override default file prefix')
        parser.add_option('-l', '--xbar_lanes', action='store', type="int",
                          default=1, help="specifies number of lanes in Xbar network")
        parser.add_option('-o', '--output_dir', action='store', type="string",
                          default='.', help='specifies output directory (default is ./)')
        parser.add_option('-g', '--gen_rtl', action='store_true',
                          default=False, help='invokes bsc compiler to generate rtl')
        parser.add_option('-x', '--run_xst', action='store_true',
                          default=False, help='generates rtl and invokes xst for synthesis')
        parser.add_option('--run_dc', action='store_true', default=False,
                          help='generates rtl and invokes Synopsys DC for synthesis')
        parser.add_option('--expose_unused_ports', action='store_true',
                          default=False, help='Exposes unused user ports for Mesh topology.')
        ...
        parser.add_option('--peek_flow_control', action='store_true', default=False,
                          help='Uses simpler peek flow control interface instead of credit-based interface.')
        ...
        parser.add_option('--voq_routers', action='store_true', default=False,
                          help='Use Virtual-Output-Queued (VOQ) routers instead of Virtual-Channel (VC) routers.')
        parser.add_option('--uni_tree_inputs', action='store',
                          type="int", default=4, help='Number of tree input ports.')
        parser.add_option('--uni_tree_outputs', action='store',
                          type="int", default=64, help='Number of tree input ports.')
        parser.add_option('--uni_tree_fanout', action='store', type="int", default=0,
                          help='Fan-out of each tree router (will be calculated automatically if set to 0).')
        parser.add_option('--uni_tree_distribute_leaves', action='store_true', default=False,
                          help='Distributes the leaf nodes to the available routers, when the tree does not perfectly fit the available leaf nodes.')
        parser.add_option('--pipeline_core', action='store_true',
                          default=False, help='Pipelines router core.')
        parser.add_option('--pipeline_alloc', action='store_true',
                          default=False, help='Pipelines router allocator.')
        parser.add_option('--pipeline_links', action='store_true',
                          default=False, help='Pipelines flit and credit links.')
        ...
        parser.add_option('--custom_topology', action='store', type="string",
                          default="", help='specifies custom topology file.')
        parser.add_option('--custom_routing', action='store', type="string",
                          default="", help='specifies custom routing file.')
        parser.add_option('--dump_topology_file', action='store_true', default=False,
                          help='dumps the topology spec file for the generated network.')
        parser.add_option('--dump_routing_file', action='store_true', default=False,
                          help='dumps the routing spec file for the generated network.')
        parser.add_option('--dbg', action='store_true', default=False,
                          help='Enables debug messages in generated rtl.')
        parser.add_option('--dbg_detail', action='store_true', default=False,
                          help='Enables more detailed debug messages in generated rtl.')
        parser.add_option('--gen_graph', action='store_true', default=False,
                          help='Visualizes network graph using graphviz.')
        parser.add_option('--graph_nodes', action='store_true', default=False,
                          help='Also includes endpoint nodes in generated graph.')
        parser.add_option('--graph_format', action='store', type="string", default="svg",
                          help='Specifies output format for graphviz (e.g., png, jpg or svg)')
        parser.add_option('--graph_layout', action='store', type="string", default="invalid",
                          help='Specifies graphviz layout engine (e.g., dot, neato, circle)')
        parser.add_option('--cdc', action='store_true', default=False,
                          help='Generates the NetworkCDC top-level wrapper with explicit independent PE clocks and sets up compilation for it.')
        parser.add_option('--cdc_depth', action='store', type="int", default=4,
                          help='Specifies the depth of the asynchronous FIFOs in the CDC wrapper (default: 4)')
        # Future parameters
